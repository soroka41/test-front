<template>
  <div class="card mt-3 mb-3">
    <div class="card-body" v-if="card">
      <p class="card-text" v-if="!showRedact">
        {{ oldCommit !== "" ? oldCommit : card.text_commit }}
      </p>
      ...
      <div class="wrap-block-comment">
        ...
        <div class="block-card-action">
          ...
          >
          <a
            @click.prevent="updateCommit(card.text_commit)"
            href=""
            class="w-100 btn btn-sm"
            :class="[!showRedact ? 'btn-primary' : 'btn-warning']"
            >{{ !showRedact ? "Редактировать" : "Сохранить" }}</a
          >
          <a
            @click.prevent="deleteCommit(cardIdx)"
            href=""
